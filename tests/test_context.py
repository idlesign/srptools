...
def test_context():
    ...
    def to_hex(val):
        return get_hexstr(val).upper()

    static_salt = int_from_hexstr('BEB25379D1A8581EB5A727673A2441EE')
    static_client_private = int_from_hexstr('60975527035CF2AD1989806F0407210BC81EDC04E2762A56AFD529DDDA2D4393')
    static_server_private = int_from_hexstr('E487CB59D31AC550471E81F00F6928E01DDA08E974A004F49E61F5D105284D20')
    ...
    password_hash = context.get_password_hash(static_salt)
    assert to_hex(password_hash) == '94B7555AABE9127CC58CCF4993DB6CF84D16C124'

    password_verifier = context.get_password_verifier(password_hash)
    assert to_hex(password_verifier) == (
    ...
    assert to_hex(common_secret) == 'CE38B9593487DA98554ED47D70A7AE5F462EF019'
    ...
    assert to_hex(server_premaster_secret) == expected_premaster_secret
    ...
    assert to_hex(client_premaster_secret) == expected_premaster_secret

    session_key = context.get_session_key(server_premaster_secret)
    assert to_hex(session_key) == '17EEFA1CEFC5C2E626E21598987F31E0F1B11BB'
